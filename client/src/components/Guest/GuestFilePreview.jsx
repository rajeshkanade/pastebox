...
const GuestFilePreview = ({ guestFiles, updateFiles }) => {
  ...
  console.log("files from preview : " , guestFiles)
  // Pagination state
  ...
  // Update parent state when deleting a file
  const deleteFile = (fileId) => {
    if (!fileId) return;
    const updatedFiles = guestFiles.filter((file) => file.id !== fileId);
  ...
  // Filtering logic uses guestFiles directly
  const filteredFiles = guestFiles?.filter((file) => {
  ...
  return (
    <div className="flex flex-col mt-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-[var(--primary-text)] mb-4">
          📁 Your Uploaded Files
        </h2>
      ...
      <div className="flex flex-col lg:flex-row gap-4 w-full lg:items-center mb-4">
        ...
        >
          ...
          {[...new Set(guestFiles?.map((f) => f.type))].map((type) => (
      ...
      {!guestFiles || guestFiles.length === 0 ? (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-[--bg-color] p-6 rounded shadow-lg w-full max-w-md md:max-w-2xl">
            ...
            <div className="mt-6 text-center">
              ...
              <img
                src={handleShare(shareFile.path).qr}
                alt="QR Code"
                className="mx-auto border rounded w-32 h-32"
              />
              <div className="flex flex-col items-center mt-4">
                <button
                  onClick={() => downloadQRCode(shareFile.path)}
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      handleShare(shareFile.path).copy
                    );
